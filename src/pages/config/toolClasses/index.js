...
const ToolClasses = props => {
  ...
  useEffect(() => {
    document.title = '工具分类'
    setLoading(true)
    toolClassess.toolClassessList({
      fun: 'asset.tool',
      node: 'root',
      mosFun: 'asset.toolClass',
      org: "*"
    })
      .then(res => {
  ...
  const destoryItem = (record) => {
    if (record !== null) {
      Modal.confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          toolClassess.toolClassessDelete(record, { _method: "DELETE" })
            .then(res => {
...
export default connect(mapStateToProps, null)(React.memo(ToolClasses))
