...
const MeterClasses = props => {
  ...
  useEffect(() => {
    document.title = '仪器仪表分类'
    setLoading(true)
    meterClassess.meterClassessList({
      fun: 'asset.meter',
      node: 'root',
      mosFun: 'asset.meterClass',
      org: user.org
    })
      .then(res => {
  ...
  const destoryItem = (record) => {
    if (record !== null) {
      Modal.confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          meterClassess.meterClassessDelete(record, { _method: "DELETE" })
            .then(res => {
...
export default connect(mapStateToProps, null)(React.memo(MeterClasses))
