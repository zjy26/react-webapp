...
const EditModel = props => {
  ...
  useEffect(() => {
    if (read) {
      ...
      meterClassess.meterClassessDetail(props.id.id)
        .then(res => {
  ...
  const handleSubmit = () => {
    form.validateFields()
      .then(values => {
        console.log(props.id)
        meterClassess.meterClassessUpdate(props.id.id, {
  ...
  return (
    ...
    >
      <Form {...layout} form={form}>
        ...
        <Form.Item name="code" label="分类代码" rules={[{ required: true, message: '请输入分类' }]}>
          <Input disabled />
        </Form.Item>
